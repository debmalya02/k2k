...
//           <p>
//             Limit : {batchDetails ? batchDetails.limitQuantity?.toString() : 'No Bottle'}
//           </p>
//           <p>
//             Quantity : {batchDetails ? batchDetails.quantity?.toString() : 'No Bottle'}
//           </p>
...
import { Textarea } from "@/components/ui/textarea";
import { Plus } from 'lucide-react'; // Optional icon for the button
import { fetchBatchDetails, fetchPackageDetails, addPackageToBatch } from '../../../../../../firebase/firebaseUtil'; // Import addPackageToBatch function
...
const BatchDetails: React.FC<Props> = ({ params }) => {
  ...
  const handleAddBottle = async () => {
    try {
      ...
      await addPackageToBatch(productId, batchId, quantityNumber);

      // Refetch package details after adding a new package
      const updatedPackageDetails = await fetchPackageDetails(productId, batchId);
  ...
  return (
    <div className="min-h-screen p-8">
      {/* Batch Number Section */}
      <div className="bg-gray-300 p-4 rounded-md mb-4 text-center">
        <h1 className="text-xl font-semibold">Batch No: {batchDetails ? batchDetails.batchNo : ''}</h1>
      </div>
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="bg-yellow-400 p-4 rounded-md">
          <h2 className="font-semibold">Batch Details</h2>
          <p>
            Limit : {batchDetails ? batchDetails.quantity?.toString() : 'No BOTTLE'}
          </p>
          <p>
        ...
        <table className="min-w-full bg-gray-300">
          ...
          <tbody>
            {packageDetails.length ? (
              ...
              <tr>
                <td colSpan={3} className="px-4 py-2 border text-center">No batch details found</td>
              </tr>
            )}
          </tbody>
        </table>
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogContent>
            ...
            <div className="mt-4 space-y-4">
              ...
              <div>
                <label htmlFor="refractometer-report" className="block text-sm font-medium">Refractometer Report</label>
                <Textarea
                  id="refractometer-report"
                  value={refractometerReport}
                  onChange={(e) => setRefractometerReport(e.target.value)}
